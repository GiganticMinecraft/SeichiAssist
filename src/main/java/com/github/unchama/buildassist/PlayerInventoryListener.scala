    ...
    if (topinventory.getTitle == s"${DARK_PURPLE.toString}${BOLD}「ブロックを並べるスキル（仮）」設定") {
      ...
      if (itemstackcurrent.getType == Material.SKULL_ITEM) {
        ...
        Coroutines.launchInGlobalScope(Schedulers.INSTANCE.getAsync, block = { case (_, cont) =>
          TargetedEffectKt.sequentialEffect[Player](
            new FocusedSoundEffect(Sound.BLOCK_FENCE_GATE_OPEN, 1f, 0.1f),
    ...
    if (topinventory.getTitle == DARK_PURPLE.toString() + "" + BOLD + "MineStackブロック一括クラフト1") {
      ...
      if (itemstackcurrent.getType == Material.SKULL_ITEM && (itemstackcurrent.getItemMeta.asInstanceOf[SkullMeta]).getOwner == "MHF_ArrowLeft") {
        ...
        if (playerdata.level < BuildAssist.config.getMinestackBlockCraftlevel(1)) {
          ...
          player.playSound(player.getLocation, Sound.BLOCK_STONE_BUTTON_CLICK_ON, 1f, 1f)
          if (playerdata_s.getMinestack.getStackedAmountOf(id_1) < Math.pow(10.0, x.toDouble).toInt) {
            player.sendMessage(RED.toString() + "アイテムが足りません")
          } else {
            playerdata_s.getMinestack.subtractStackedAmountOf(id_1, Math.pow(10.0, x.toDouble).toInt.toLong)
        ...
        if (playerdata.level < BuildAssist.config.getMinestackBlockCraftlevel(2)) {
          ...
          player.playSound(player.getLocation, Sound.BLOCK_STONE_BUTTON_CLICK_ON, 1f, 1f)
          if (playerdata_s.getMinestack.getStackedAmountOf(id_1) < Math.pow(10.0, x.toDouble).toInt * 4) {
            player.sendMessage(RED.toString() + "アイテムが足りません")
          } else {
            playerdata_s.getMinestack.subtractStackedAmountOf(id_1, (Math.pow(10.0, x.toDouble).toInt * 4).toLong)
            playerdata_s.getMinestack.addStackedAmountOf(id_2, Math.pow(10.0, x.toDouble).toInt.toLong)
    ...
    if (topinventory.getTitle == DARK_PURPLE.toString() + "" + BOLD + "MineStackブロック一括クラフト2") {
      ...
      if (itemstackcurrent.getType == Material.SKULL_ITEM && (itemstackcurrent.getItemMeta.asInstanceOf[SkullMeta]).getOwner == "MHF_ArrowUp") {
        ...
        if (playerdata.level < BuildAssist.config.getMinestackBlockCraftlevel(2)) {
          ...
          player.playSound(player.getLocation, Sound.BLOCK_STONE_BUTTON_CLICK_ON, 1f, 1f)
          if (playerdata_s.getMinestack.getStackedAmountOf(id_1) < Math.pow(10.0, x.toDouble).toInt * 2 || playerdata_s.getMinestack.getStackedAmountOf(id_3) < Math.pow(10.0, x.toDouble).toInt * 2) {
            player.sendMessage(RED.toString() + "アイテムが足りません")
          } else {
            playerdata_s.getMinestack.subtractStackedAmountOf(id_1, (Math.pow(10.0, x.toDouble).toInt * 2).toLong)
            playerdata_s.getMinestack.subtractStackedAmountOf(id_3, (Math.pow(10.0, x.toDouble).toInt * 2).toLong)
            playerdata_s.getMinestack.addStackedAmountOf(id_2, Math.pow(10.0, x.toDouble).toInt.toLong)
        ...
        if (playerdata.level < BuildAssist.config.getMinestackBlockCraftlevel(3)) {
          ...
          player.playSound(player.getLocation, Sound.BLOCK_STONE_BUTTON_CLICK_ON, 1f, 1f)
          if (playerdata_s.getMinestack.getStackedAmountOf(id_1) < Math.pow(10.0, x.toDouble).toInt * 4 || playerdata_s.getMinestack.getStackedAmountOf(id_3) < Math.pow(10.0, x.toDouble).toInt) {
            player.sendMessage(RED.toString() + "アイテムが足りません")
          } else {
            playerdata_s.getMinestack.subtractStackedAmountOf(id_1, (Math.pow(10.0, x.toDouble).toInt * 4).toLong)
            playerdata_s.getMinestack.subtractStackedAmountOf(id_3, Math.pow(10.0, x.toDouble).toInt.toLong)
        ...
        if (playerdata.level < BuildAssist.config.getMinestackBlockCraftlevel(3)) {
          ...
          player.playSound(player.getLocation, Sound.BLOCK_STONE_BUTTON_CLICK_ON, 1f, 1f)
          if (playerdata_s.getMinestack.getStackedAmountOf(id_1) < Math.pow(10.0, x.toDouble).toInt * 50 || playerdata_s.getMinestack.getStackedAmountOf(id_3) < Math.pow(10.0, x.toDouble).toInt) {
            player.sendMessage(RED.toString() + "アイテムが足りません")
          } else {
            playerdata_s.getMinestack.subtractStackedAmountOf(id_1, (Math.pow(10.0, x.toDouble).toInt * 50).toLong)
            playerdata_s.getMinestack.subtractStackedAmountOf(id_3, Math.pow(10.0, x.toDouble).toInt.toLong)
            playerdata_s.getMinestack.addStackedAmountOf(id_2, (Math.pow(10.0, x.toDouble).toInt * 50).toLong)
