...
public class PlayerDataTableQueryGenerator implements TableInitializationQueryGenerator {
    ...
    public String generateColumnCreationQuery() {
        //必要なcolumnを随時追加
        // 多分、どうあがいても1000字以上はある
        StringBuilder command = new StringBuilder(1000);
                command.append("alter table " + tableReferenceName +
        ...
        command.append(",add column if not exists playtick int default 0" + ",add column if not exists killlogflag boolean default false" + ",add column if not exists worldguardlogflag boolean default true" +//

                ",add column if not exists multipleidbreakflag boolean default false" + ",add column if not exists pvpflag boolean default false" + ",add column if not exists loginflag boolean default false" + ",add column if not exists p_vote int default 0" + ",add column if not exists p_givenvote int default 0" + ",add column if not exists effectpoint int default 0" + ",add column if not exists premiumeffectpoint int default 0" + ",add column if not exists mana double default 0.0" + ",add column if not exists expvisible boolean default true" + ",add column if not exists totalexp int default 0" + ",add column if not exists expmarge tinyint unsigned default 0" + ",add column if not exists shareinv blob" + ",add column if not exists everysound boolean default true" + ",add column if not exists everymessage boolean default true" + ",add column if not exists homepoint_").append(config.getServerNum()).append(" varchar(").append(config.getSubHomeMax() * SeichiAssist.SUB_HOME_DATASIZE).append(") default ''").append(",add column if not exists subhome_name_").append(config.getServerNum()).append(" blob default null").append(


                //BuildAssistのデータ
                ",add column if not exists build_lv int default 1").append(//
                ",add column if not exists build_count double default 0").append(//
                ",add column if not exists build_count_flg TINYINT UNSIGNED default 0").append(//

                ",add column if not exists anniversary boolean default false").append(//

                //投票妖精関連
                ",add column if not exists canVotingFairyUse boolean default false").append(//
                ",add column if not exists newVotingFairyTime varchar(").append(SeichiAssist.VOTE_FAIRYTIME_DATASIZE).append(") default ''").append(//
                ",add column if not exists VotingFairyRecoveryValue int default 0").append(//
                ",add column if not exists hasVotingFairyMana int default 0").append(//
                ",add column if not exists toggleGiveApple int default 1").append(//
                ",add column if not exists toggleVotingFairy int default 1").append(//
                ",add column if not exists p_apple bigint default 0").append(

                //貢献pt関連
                ",add column if not exists contribute_point int default 0").append(//
                ",add column if not exists added_mana int default 0").append(
                ",add column if not exists lastvote varchar(40) default null").append(
                ",add column if not exists chainvote int default 0").append(
                ",add column if not exists GBstage int default 0").append(
                ",add column if not exists GBexp int default 0").append(
                ",add column if not exists GBlevel int default 0").append(
                ",add column if not exists isGBStageUp boolean default false");

        for (int i = 0; i <= config.getTemplateKeepAmount() - 1; i++) {
            command.append(",add column if not exists ahead_").append(i).append(" int default 0");
            command.append(",add column if not exists behind_").append(i).append(" int default 0");
            command.append(",add column if not exists right_").append(i).append(" int default 0");
            command.append(",add column if not exists left_").append(i).append(" int default 0");
        }
        ...
        for (final ActiveSkillEffect activeSkillEffect : activeSkillEffects) {
            command.append(",add column if not exists ").append(activeSkillEffect.getsqlName()).append(" boolean default false");
        }
        ...
        for (final ActiveSkillPremiumEffect activeSkillPremiumEffect : premiumEffects) {
            command.append(",add column if not exists ").append(activeSkillPremiumEffect.getsqlName()).append(" boolean default false");
        }

        //正月Event用
        command.append(",add column if not exists hasNewYearSobaGive boolean default false");
        command.append(",add column if not exists newYearBagAmount int default 0");

        //バレンタインイベント用
        command.append(",add column if not exists hasChocoGave boolean default false");
