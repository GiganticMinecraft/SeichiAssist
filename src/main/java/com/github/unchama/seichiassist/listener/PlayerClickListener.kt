    ...
    if (action === Action.RIGHT_CLICK_AIR || action === Action.RIGHT_CLICK_BLOCK) {
      ...
      if (mainhandflag && equipmentslot === EquipmentSlot.HAND) {
        ...
        if (skillTypeId == ActiveSkill.BREAK.gettypenum() && skillNumber == 1 || skillTypeId == ActiveSkill.BREAK.gettypenum() && skillNumber == 2) {
          ...
          player.sendMessage(ChatColor.GOLD.toString() + ActiveSkill.getActiveSkillName(skillTypeId, skillNumber) + status)
          ...
          when (activemineflagnum) {
            0 -> player.sendMessage(ChatColor.GOLD.toString() + ActiveSkill.getActiveSkillName(skillTypeId, skillNumber) + "：OFF")
            1 -> player.sendMessage(ChatColor.GOLD.toString() + ActiveSkill.getActiveSkillName(skillTypeId, skillNumber) + ":ON")
          }
          playerdata.activeskilldata.updateSkill(player, skillTypeId, skillNumber, activemineflagnum)
          player.playSound(player.location, Sound.BLOCK_LEVER_CLICK, 1f, 1f)
        }
      ...
      if (player.inventory.itemInOffHand.type in MaterialSets.breakMaterials && equipmentslot === EquipmentSlot.OFF_HAND) {
        ...
        if (assaultNumber >= 4 && assaultTypeId >= 4) {
          ...
          if (activemineflagnum == 0) {
            player.sendMessage(ChatColor.GOLD.toString() + ActiveSkill.getActiveSkillName(assaultTypeId, assaultNumber) + ":OFF")
          } else {
            player.sendMessage(ChatColor.GOLD.toString() + ActiveSkill.getActiveSkillName(assaultTypeId, assaultNumber) + ":ON")
          }
