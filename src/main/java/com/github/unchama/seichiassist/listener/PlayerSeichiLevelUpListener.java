...
public class PlayerSeichiLevelUpListener {
	...
	public void onPlayerLevelUp(SeichiLevelUpEvent event) {
		...
		switch (event.getLevelAfterLevelUp()) {
			case 10: Util.INSTANCE.addItemToPlayerSafely(p, ItemData.getSuperPickaxe(5));
			case 20: {
				GachaCommand.Gachagive(p, 3, p.getName());
				GachaCommand.Gachagive(p, 10, p.getName());
			}
			case 30: Util.INSTANCE.addItemToPlayerSafely(p, ItemData.getForLevelUpskull(p.getName(), 256));
			case 40: Util.INSTANCE.addItemToPlayerSafely(p, ItemData.getGachaApple(256));
			...
			case 100: {
				...
				Util.INSTANCE.addItemToPlayerSafely(p, ItemData.getElsa(1));
			}
		}
	}
}
