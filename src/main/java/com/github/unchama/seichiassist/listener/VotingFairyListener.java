...
public class VotingFairyListener implements Listener {
	...
	public static void summon(Player p){
		...
		if (Util.getTimeZone(playerdata.VotingFairyStartTime).equals("morning"))
			VotedFairyTask.speak(p, getMessage(morning, p.getName()), playerdata.toggleVFSound);
		else if (Util.getTimeZone(playerdata.VotingFairyStartTime).equals("day"))
			VotedFairyTask.speak(p, getMessage(day, p.getName()), playerdata.toggleVFSound);
		else
			VotedFairyTask.speak(p, getMessage(night, p.getName()), playerdata.toggleVFSound);
	}
	...
	public static void regeneMana(Player p) {
		...
		if (mana.getMana() == mana.getMax()) {		//マナが最大だった場合はメッセージを送信して終わり
			...
			VotedFairyTask.speak(p, getMessage(msg, p.getName()), playerdata.toggleVFSound);
			...
			if (m != 0) {
				...
				VotedFairyTask.speak(p, getMessage(yes, p.getName()), playerdata.toggleVFSound);
			}else {

				p.sendMessage(ChatColor.RESET + "" + ChatColor.YELLOW + "" + ChatColor.BOLD + "あなたは妖精にりんごを渡しませんでした。");
				VotedFairyTask.speak(p, getMessage(no, p.getName()), playerdata.toggleVFSound);
			}
		}
