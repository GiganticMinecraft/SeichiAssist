...
public class PlayerClickListener implements Listener {
	...
	public void onPlayerActiveSkillEvent(PlayerInteractEvent event){
		...
		if(playerdata.getActiveskilldata().mineflagnum == 0 || playerdata.getActiveskilldata().skillnum == 0){
			return;
		}
		...
		if(action == Action.RIGHT_CLICK_AIR || action == Action.RIGHT_CLICK_BLOCK){
			...
			if(playerdata.getActiveskilldata().assaulttype!=0){
				return;
			}
			//クールダウンタイム中は処理を終了
	...
	public void onPlayerActiveSkillToggleEvent(PlayerInteractEvent event){
		...
		if(action == Action.RIGHT_CLICK_AIR || action == Action.RIGHT_CLICK_BLOCK){
			...
			if(mainhandflag && equipmentslot == EquipmentSlot.HAND){
				...
				if(!player.isSneaking() && playerdata.getActiveskilldata().assaulttype == 0){
					return;
				}
				...
				if((playerdata.getActiveskilldata().skilltype == ActiveSkill.BREAK.gettypenum() && playerdata.getActiveskilldata().skillnum == 1)
						|| (playerdata.getActiveskilldata().skilltype == ActiveSkill.BREAK.gettypenum() && playerdata.getActiveskilldata().skillnum == 2)){

					activemineflagnum = (activemineflagnum + 1) % 3;
					switch (activemineflagnum){
					case 0:
						player.sendMessage(ChatColor.GOLD + ActiveSkill.getActiveSkillName(playerdata.getActiveskilldata().skilltype, playerdata.getActiveskilldata().skillnum) + "：OFF");
						break;
					case 1:
						player.sendMessage(ChatColor.GOLD + ActiveSkill.getActiveSkillName(playerdata.getActiveskilldata().skilltype, playerdata.getActiveskilldata().skillnum) + ":ON-Above(上向き）");
						break;
					case 2:
						player.sendMessage(ChatColor.GOLD + ActiveSkill.getActiveSkillName(playerdata.getActiveskilldata().skilltype, playerdata.getActiveskilldata().skillnum) + ":ON-Under(下向き）");
						break;
					}
					playerdata.getActiveskilldata().updateSkill(player, playerdata.getActiveskilldata().skilltype, playerdata.getActiveskilldata().skillnum, activemineflagnum);
					player.playSound(player.getLocation(), Sound.BLOCK_LEVER_CLICK, 1, 1);
				}else if(playerdata.getActiveskilldata().skilltype > 0 && playerdata.getActiveskilldata().skillnum > 0
						&& playerdata.getActiveskilldata().skilltype < 4
						){
					...
					case 0:
						...
						break;
					case 1:
						player.sendMessage(ChatColor.GOLD + ActiveSkill.getActiveSkillName(playerdata.getActiveskilldata().skilltype, playerdata.getActiveskilldata().skillnum) + ":ON");
						break;
					}
					playerdata.getActiveskilldata().updateSkill(player, playerdata.getActiveskilldata().skilltype, playerdata.getActiveskilldata().skillnum, activemineflagnum);
					player.playSound(player.getLocation(), Sound.BLOCK_LEVER_CLICK, 1, 1);
			...
			if(SeichiAssist.breakmateriallist.contains(player.getInventory().getItemInOffHand().getType())
				...
				if(playerdata.getActiveskilldata().assaultnum >=4 && playerdata.getActiveskilldata().assaulttype >=4){
					//メインハンドでも指定ツールを持っていたらフラグは変えない
					if(!mainhandflag || playerdata.getActiveskilldata().skillnum == 0){
						activemineflagnum = (activemineflagnum + 1) % 2;
					}
					if(activemineflagnum == 0){
						player.sendMessage(ChatColor.GOLD + ActiveSkill.getActiveSkillName(playerdata.getActiveskilldata().assaulttype, playerdata.getActiveskilldata().assaultnum) + ":OFF");
					}else{
						player.sendMessage(ChatColor.GOLD + ActiveSkill.getActiveSkillName(playerdata.getActiveskilldata().assaulttype, playerdata.getActiveskilldata().assaultnum) + ":ON");
					}
					playerdata.getActiveskilldata().updateAssaultSkill(player, playerdata.getActiveskilldata().assaulttype, playerdata.getActiveskilldata().assaultnum,activemineflagnum);
					player.playSound(player.getLocation(), Sound.BLOCK_LEVER_CLICK, 1, 1);
				}
			}
		}
