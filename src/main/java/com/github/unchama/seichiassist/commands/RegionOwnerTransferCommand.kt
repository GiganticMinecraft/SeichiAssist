...
object RegionOwnerTransferCommand {
  ...
  private suspend fun attemptRegionTransfer(donner: Player, recipient: Player, region: ProtectedRegion): ResponseToSender {
    ...
    if (!owners.contains(donner.uniqueId)) {
      return "オーナーではないため権限を譲渡できません。".asResponseToSender()
    }

    if (owners.size() != 1) {
      return "オーナーが複数人いるため権限を譲渡できません。".asResponseToSender()
    }

    owners.clear()
    owners.addPlayer(recipient.uniqueId)

    return "${recipient.name}に${region.id}のオーナー権限を譲渡しました。".asResponseToSender()
  }
