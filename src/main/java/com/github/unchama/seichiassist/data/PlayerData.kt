...
class PlayerData(val player: Player) {
    ...
    fun dispPassiveExp(): Double {
        return when {
          level < 8 -> 0.0
          level < 18 -> SeichiAssist.config.getDropExplevel(1)
          level < 28 -> SeichiAssist.config.getDropExplevel(2)
          level < 38 -> SeichiAssist.config.getDropExplevel(3)
          level < 48 -> SeichiAssist.config.getDropExplevel(4)
          level < 58 -> SeichiAssist.config.getDropExplevel(5)
          level < 68 -> SeichiAssist.config.getDropExplevel(6)
          level < 78 -> SeichiAssist.config.getDropExplevel(7)
          level < 88 -> SeichiAssist.config.getDropExplevel(8)
          level < 98 -> SeichiAssist.config.getDropExplevel(9)
          else -> SeichiAssist.config.getDropExplevel(10)
        }
    }
