...
public class RegionMenuData {
	...
	public static Inventory getRegionMenuData(Player player) {
		...
		//0マス目
		List<String> lore0 = ReadonlyListFactory.INSTANCE.of(ChatColor.RESET + "" +  ChatColor.DARK_RED + "" + ChatColor.UNDERLINE + "クリックで召喚"
		...
		if(!player.hasPermission("worldguard.region.claim")){
			lore1.addAll(ReadonlyListFactory.INSTANCE.of(ChatColor.RED + "このワールドでは"
					, ChatColor.RED + "保護を申請出来ません"
			));
		}else if (selection == null) {
			lore1.addAll(ReadonlyListFactory.INSTANCE.of(ChatColor.RED + "範囲指定されてません"
					, ChatColor.RED + "先に木の斧で2か所クリックしてネ"
			));
		}else if(selection.getLength() < 10||selection.getWidth() < 10){
			lore1.addAll(ReadonlyListFactory.INSTANCE.of(ChatColor.RED + "選択された範囲が狭すぎます"
					, ChatColor.RED + "1辺当たり最低10ブロック以上にしてネ"
			));
		}else{
			itemmeta1.addEnchant(Enchantment.DIG_SPEED, 100, false);
			lore1.addAll(ReadonlyListFactory.INSTANCE.of(ChatColor.DARK_GREEN + "" + ChatColor.UNDERLINE + "範囲指定されています"
		...
		if(player.hasPermission("worldguard.region.claim")){
			...
			lore1.addAll(ReadonlyListFactory.INSTANCE.of(ChatColor.DARK_GRAY + "Y座標は自動で全範囲保護されます"
		...
		//2マス目
		List<String> lore2 =  ReadonlyListFactory.INSTANCE.of(ChatColor.RESET + "" +  ChatColor.DARK_RED + "" + ChatColor.UNDERLINE + "クリックで表示"
		...
		//3マス目
		List<String> lore3 = ReadonlyListFactory.INSTANCE.of(ChatColor.RESET + "" +  ChatColor.DARK_RED + "" + ChatColor.UNDERLINE + "クリックで開く"
		...
		//4マス目
		List<String> lore4 = ReadonlyListFactory.INSTANCE.of(ChatColor.RESET + "" + ChatColor.DARK_RED + "" + ChatColor.UNDERLINE + "クリックで開く"
