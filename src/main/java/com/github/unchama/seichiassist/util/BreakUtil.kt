...
object BreakUtil {
  ...
  fun canBreak(player: Player, breakblock: Block?): Boolean {
  ...
  private fun equalsIgnoreNameCaseWorld(name: String): Boolean {
  ...
  fun breakBlock(player: Player, breakblock: Block, centerofblock: Location, tool: ItemStack, stepflag: Boolean) {
  ...
  fun addItemToMineStack(player: Player, itemstack: ItemStack): Boolean {
    //もしサバイバルでなければ処理を終了
    ...
    MineStackObjectList.minestacklist!!.forEach { mineStackObj ->
      fun addToMineStackAfterLevelCheck(): Boolean =
  ...
  fun dropItemOnTool(breakblock: Block, tool: ItemStack): ItemStack? {
  ...
  fun calcManaDrop(playerdata: PlayerData): Double {
    //０～１のランダムな値を取得
    val rand = Math.random()
    //10%の確率で経験値付与
    return if (rand < 0.1) {
  ...
  //num回だけ耐久を減らす処理
  fun calcDurability(enchantmentLevel: Int, num: Int): Short {
    val rand = Random()
  ...
  fun getCardinalDirection(entity: Entity): String? {
   ...
   */
  fun getGravity(player: Player, block: Block, isAssault: Boolean): Int {
  ...
  fun logRemove(player: Player, removedBlock: Block): Boolean {
    val wrapper = ExternalPlugins.getCoreProtectWrapper()
    if (wrapper == null) {
