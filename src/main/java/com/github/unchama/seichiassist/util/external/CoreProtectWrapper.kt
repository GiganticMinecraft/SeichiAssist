...
class CoreProtectWrapper(val backbone: CoreProtectAPI) {
  fun queueBlockRemoval(who: Player, where: Location, rawType: Byte): Boolean {
    return backbone.logRemoval(who.name, where, where.block.type, rawType)
  }

  fun queueBlockRemoval(who: Player, where: Block): Boolean {
    return backbone.logRemoval(who.name, where.location, where.type, where.data)
  }
  /*
  // For >= 1.13
  fun queueBlockRemoval(who: Player, where: Location, data: BlockData): Boolean {
