...
public class SeichiAssist extends JavaPlugin{
	...
	private static final List<MineStackMineObj> minestacklistmine = Arrays.asList(
			...
			,new MineStackMineObj("snow_layer","雪タイル",1,Material.SNOW,0)
			,new MineStackMineObj("snow_ball","雪玉",1,Material.SNOW_BALL,0)
	...
	private static final List<MineStackDropObj> minestacklistdrop = Arrays.asList(
			//以下モンスター+動物ドロップ
			...
			,new MineStackDropObj("cooked_porkchop","焼き豚",1,Material.GRILLED_PORK,0)

			,new MineStackDropObj("raw_chicken","生の鶏肉",1,Material.RAW_CHICKEN,0)
			,new MineStackDropObj("cooked_chicken","焼き鳥",1,Material.COOKED_CHICKEN,0)
			...
			//2019.2追加分
			,new MineStackDropObj("bread", "パン", 1, Material.BREAD, 0)
			,new MineStackDropObj("sugar", "砂糖", 1, Material.SUGAR, 0)
			,new MineStackDropObj("baked_potato", "ベイクドポテト", 1, Material.BAKED_POTATO, 0)
			,new MineStackDropObj("cake", "ケーキ", 1, Material.CAKE, 0)
			,new MineStackDropObj("mushroom_stew", "キノコシチュー", 1, Material.MUSHROOM_SOUP, 0)
			,new MineStackDropObj("rabbit_stew", "ウサギシチュー", 1, Material.RABBIT_STEW, 0)
	...
	private static final List<MineStackFarmObj> minestacklistfarm = Arrays.asList(
			//以下採掘で入手可能な農業系ブロック
	...
	private static final List<MineStackBuildObj> minestacklistbuild = Arrays.asList(
			...
			,new MineStackBuildObj("stained_clay","橙色の堅焼き粘土",2,Material.STAINED_CLAY,0)
			...
			,new MineStackBuildObj("stained_clay15","黒色の堅焼き粘土",1,Material.STAINED_CLAY,15)
	...
	private static final List<MineStackRsObj> minestacklistrs = Arrays.asList(
	...
	private static List<MineStackGachaObj> minestacklistgacha = Arrays.asList(
	...
	public void onEnable(){
		...
		if (sql.loadMineStackGachaData()) { //MineStack用ガチャデータを読み込んだ
			...
			minestacklistfarm.forEach(MineStackRegistry::addFarmMaterial);
			minestacklistrs.forEach(MineStackRegistry::addRedstoneMaterial);
