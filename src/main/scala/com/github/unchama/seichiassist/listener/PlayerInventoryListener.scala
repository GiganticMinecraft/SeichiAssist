...
class PlayerInventoryListener(
  ...
  def onGachaTradeEvent(event: InventoryCloseEvent): Unit = {
    ...
    if (inventory.getTitle == s"${LIGHT_PURPLE.toString}${BOLD}交換したい景品を入れてください") {
      ...
      while (givegacha > 0) {
        if (
          player.getInventory.contains(skull) || !InventoryUtil.isPlayerInventoryFull(player)
        ) {
  ...
  def onGachaRingoEvent(event: InventoryCloseEvent): Unit = {
    ...
    if (inventory.getTitle == GOLD.toString + "" + BOLD + "椎名林檎と交換したい景品を入れてネ") {
      ...
      while (giveringo > 0) {
        if (
          player.getInventory.contains(ringo) || !InventoryUtil.isPlayerInventoryFull(player)
        ) {
    ...
    if (topinventory.getTitle == DARK_PURPLE.toString + "" + BOLD + "投票ptメニュー") {
      ...
      if (itemstackcurrent.getType == Material.DIAMOND) {
        ...
        while (n > 0) {
          ...
          for { _ <- 0 to 9 } {
            if (
              player.getInventory.contains(skull) || !InventoryUtil.isPlayerInventoryFull(
                player
              )
            ) {
              InventoryUtil.addItem(player, skull)
