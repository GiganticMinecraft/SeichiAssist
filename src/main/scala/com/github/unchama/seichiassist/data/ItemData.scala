...
object ItemData {
  def getSuperPickaxe(amount: Int): ItemStack = {
    val itemstack: ItemStack = new ItemStack(Material.DIAMOND_PICKAXE, amount)
    val itemmeta: ItemMeta = Bukkit.getItemFactory.getItemMeta(Material.DIAMOND_PICKAXE)
    itemmeta.setDisplayName(ChatColor.YELLOW + "" + ChatColor.BOLD + "Thanks for Voting!")
    val lore = List("投票ありがとナス♡")
  ...
  def getForLevelUpskull(name: String, amount: Int): ItemStack = {
    ...
    val lore = List(ChatColor.RESET + "" + ChatColor.GREEN + "右クリックで使えます", ChatColor.RESET + "" + ChatColor.DARK_GREEN + "所有者：" + name, ChatColor.RESET + "" + ChatColor.DARK_RED + "レベルアップ記念です")
  ...
  def getElsa(amount: Int): ItemStack = {
    ...
    val lore = List("", ChatColor.GREEN + "装備中の移動速度" + ChatColor.YELLOW + "(中)" + ChatColor.GREEN, "", ChatColor.YELLOW + "金床" + ChatColor.RED + "不可", ChatColor.YELLOW + "修繕エンチャント" + ChatColor.AQUA + "可")
    elsaMeta.setLore(lore.asJava)
  ...
  def getVotingGift(amount: Int): ItemStack = {
    ...
    val lore = List("", ChatColor.WHITE + "公共施設鯖にある", ChatColor.WHITE + "デパートで買い物ができます")
    itemMeta.setLore(lore.asJava)
