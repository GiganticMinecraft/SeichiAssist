...
object MenuInventoryData { // 実際には60人も入ることは無いのでは？
  ...
  private val loreTable = util.Arrays.asList(Collections.emptyList, util.Arrays.asList(RED + "" + UNDERLINE + "" + BOLD + "ガンガンたべるぞ", RESET + "" + GRAY + "とにかく妖精さんにりんごを開放します。", RESET + "" + GRAY + "めっちゃ喜ばれます。"), util.Arrays.asList(YELLOW + "" + UNDERLINE + "" + BOLD + "バッチリたべよう", RESET + "" + GRAY + "食べ過ぎないように注意しつつ", RESET + "" + GRAY + "妖精さんにりんごを開放します。", RESET + "" + GRAY + "喜ばれます。"), util.Arrays.asList(GREEN + "" + UNDERLINE + "" + BOLD + "リンゴだいじに", RESET + "" + GRAY + "少しだけ妖精さんにりんごを開放します。", RESET + "" + GRAY + "伝えると大抵落ち込みます。"), util.Arrays.asList(BLUE + "" + UNDERLINE + "" + BOLD + "リンゴつかうな", RESET + "" + GRAY + "絶対にりんごを開放しません。", RESET + "" + GRAY + ""))
  /**
  ...
  private val toMoveNicknameMenu = build(Material.BARRIER, s"$YELLOW$UNDERLINE${BOLD}二つ名組合せメインメニューへ", s"$RESET${DARK_RED}${UNDERLINE}クリックで移動")

  //投票特典受け取りボタン
  private def getVoteButtonLore(playerdata: PlayerData) = util.Arrays.asList(RESET + "" + GRAY + "投票特典を受け取るには", RESET + "" + GRAY + "投票ページで投票した後", RESET + "" + GRAY + "このボタンをクリックします", RESET + "" + AQUA + "特典受取済投票回数：" + playerdata.p_givenvote, RESET + "" + AQUA + "所有投票pt：" + playerdata.effectPoint)
  ...
  def getRankingBySeichiAmount(page: Int): Inventory = {
    ...
    val lowerBound = 100
    val inventory = getEmptyInventory(6, DARK_PURPLE + "" + BOLD + "整地神ランキング")
    ...
    (perPage * page until perPage + perPage * page)
      .foreach(rank => {
        ...
        val lore = util.Arrays.asList(RESET + "" + GREEN + "整地Lv:" + rankdata.level, RESET + "" + GREEN + "総整地量:" + rankdata.totalbreaknum)
        val skullmeta = buildSkullMeta(YELLOW + "" + BOLD + "" + (rank + 1) + "位:" + "" + WHITE + rankdata.name, lore, rankdata.name)
        itemstack.setItemMeta(skullmeta)
        AsyncInventorySetter.setItemAsync(inventory, invIndex, itemstack.clone)
        invIndex += 1
      })
    if (page != pageLimit) { // 整地神ランキング次ページ目を開く
      val skullMeta = buildSkullMeta(YELLOW + "" + UNDERLINE + "" + BOLD + "整地神ランキング" + (page + 2) + "ページ目へ", Collections.singletonList(RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで移動"), "MHF_ArrowDown")
      itemstack.setItemMeta(skullMeta)
    ...
    val (name, lore, ign) = if (page == 0) {
      ...
      (
        ...
        Collections.singletonList(RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで移動"),
        "MHF_ArrowUp"
      )
    }
    ...
    // 総整地量の表記
    val lore = util.Arrays.asList(RESET + "" + AQUA + "全プレイヤー総整地量:", RESET + "" + AQUA + SeichiAssist.allplayerbreakblockint)
    val skullmeta = buildSkullMeta(YELLOW + "" + UNDERLINE + "" + BOLD + "整地鯖統計データ", lore, "unchama")
    itemstack.setItemMeta(skullmeta)
  ...
  def getRankingByPlayingTime(page: Int): Inventory = {
    val pageLimit = 14
    val inventory = getEmptyInventory(6, DARK_PURPLE + "" + BOLD + "ログイン神ランキング")
    val itemstack = new ItemStack(Material.SKULL_ITEM, 1, PLAYER_SKULL)
    ...
    for (rank <- rankStart until rankStart + 10) {
      ...
      val skullmeta = buildSkullMeta(s"$YELLOW$BOLD${rank + 1}位:$WHITE${rankdata.name}", Collections.singletonList(RESET + "" + GREEN + "総ログイン時間:" + TypeConverter.toTimeString(TypeConverter.toSecond(rankdata.playtick))), rankdata.name)
      itemstack.setItemMeta(skullmeta)
      AsyncInventorySetter.setItemAsync(inventory, invIndex, itemstack.clone)
      invIndex += 1
    }
    if (page != pageLimit) {
      val skullmeta = buildSkullMeta(YELLOW + "" + UNDERLINE + "" + BOLD + "ログイン神ランキング" + (page + 2) + "ページ目へ", Collections.singletonList(RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで移動"), "MHF_ArrowDown")
      itemstack.setItemMeta(skullmeta)
      AsyncInventorySetter.setItemAsync(inventory, 52, itemstack.clone)
    }
    // 前のページ / ホームへ
    var skullmeta = null
    if (page == 0) skullmeta = buildSkullMeta(YELLOW + "" + UNDERLINE + "" + BOLD + "ホームへ", Collections.singletonList(RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで移動"), "MHF_ArrowLeft")
    else { // 整地神ランキング前ページを開く;
      skullmeta = buildSkullMeta(YELLOW + "" + UNDERLINE + "" + BOLD + "ログイン神ランキング" + page + "ページ目へ", Collections.singletonList(RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで移動"), "MHF_ArrowUp")
    }
    itemstack.setItemMeta(skullmeta)
    AsyncInventorySetter.setItemAsync(inventory, 45, itemstack.clone)
  ...
  def getRankingByVotingCount(page: Int): Inventory = {
    val pageLimit = 14
    val inventory = getEmptyInventory(6, DARK_PURPLE + "" + BOLD + "投票神ランキング")
    val itemstack = new ItemStack(Material.SKULL_ITEM, 1, PLAYER_SKULL)
    ...
    while ( {
      ...
      val skullmeta = buildSkullMeta(YELLOW + "" + BOLD + "" + (voteRank + 1) + "位:" + "" + WHITE + rankdata.name, Collections.singletonList(RESET + "" + GREEN + "総投票回数:" + rankdata.p_vote), rankdata.name)
      itemstack.setItemMeta(skullmeta)
      AsyncInventorySetter.setItemAsync(inventory, invIndex, itemstack.clone)
    ...
    if (page != pageLimit) { // 投票神ランキング次ページ目を開く
      val skullmeta = buildSkullMeta(YELLOW + "" + UNDERLINE + "" + BOLD + "投票神ランキング" + (page + 2) + "ページ目へ", Collections.singletonList(RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで移動"), "MHF_ArrowDown")
      itemstack.setItemMeta(skullmeta)
      AsyncInventorySetter.setItemAsync(inventory, 52, itemstack.clone)
    }
    var skullmeta = null
    if (page == 0) skullmeta = buildSkullMeta(YELLOW + "" + UNDERLINE + "" + BOLD + "ホームへ", Collections.singletonList(RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで移動"), "MHF_ArrowLeft")
      ...
      skullmeta = buildSkullMeta(YELLOW + "" + UNDERLINE + "" + BOLD + "投票神ランキング" + page + "ページ目へ", Collections.singletonList(RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで移動"), "MHF_ArrowUp")
    }
    itemstack.setItemMeta(skullmeta)
    AsyncInventorySetter.setItemAsync(inventory, 45, itemstack.clone)
  ...
  def computeRefreshedCombineMenu(p: Player): Inventory = {
    ...
    taihiIndex.put(uuid, 0);
    {
      //実績ポイントの最新情報反映ボタン
      // dynamic button
      val lore = util.Arrays.asList(RESET + "" + GREEN + "クリックで情報を最新化", RESET + "" + RED + "累計獲得量：" + playerdata.achievePoint.cumulativeTotal, RESET + "" + RED + "累計消費量：" + playerdata.achievePoint.used, RESET + "" + AQUA + "使用可能量：" + playerdata.achievePoint.left)
      val itemstack = build(Material.EMERALD_ORE, YELLOW + "" + UNDERLINE + "" + BOLD + "実績ポイント 情報", lore)
...

    {
      // パーツショップ
      // const button
      val itemstack = build(Material.ITEM_FRAME, YELLOW + "" + UNDERLINE + "" + BOLD + "実績ポイントショップ", RESET + "" + GREEN + "クリックで開きます")
      AsyncInventorySetter.setItemAsync(inventory, 9, itemstack)
    }
    {
      //エフェクトポイントからの変換ボタン
      val lore = util.Arrays.asList(RESET + "" + RED + "JMS投票で手に入るポイントを", RESET + "" + RED + "実績ポイントに変換できます。", RESET + "" + YELLOW + "" + BOLD + "投票pt 10pt → 実績pt 3pt", RESET + "" + AQUA + "クリックで変換を一回行います。", RESET + "" + GREEN + "所有投票pt :" + playerdata.effectPoint, RESET + "" + GREEN + "所有実績pt :" + playerdata.achievePoint.left)
      val itemstack = build(Material.EMERALD, YELLOW + "" + UNDERLINE + "" + BOLD + "ポイント変換ボタン", lore)
    ...
    }
    {
      ...
      val itemStack = build(Material.BOOK, YELLOW + "" + UNDERLINE + "" + BOLD + "現在の二つ名の確認", RESET + "" + RED + "「" + playerTitle + "」")
      AsyncInventorySetter.setItemAsync(inventory, 4, itemStack)
    }
    {
      val toHeadSelection = build(Material.WATER_BUCKET, YELLOW + "" + UNDERLINE + "" + BOLD + "前パーツ選択画面", RESET + "" + RED + "クリックで移動します")
      val toMiddleSelection = build(Material.MILK_BUCKET, YELLOW + "" + UNDERLINE + "" + BOLD + "中パーツ選択画面", RESET + "" + RED + "クリックで移動します")
      val toTailSelection = build(Material.LAVA_BUCKET, YELLOW + "" + UNDERLINE + "" + BOLD + "後パーツ選択画面", RESET + "" + RED + "クリックで移動します")
    ...
    {
      // const Button
      val itemstack = buildPlayerSkull(YELLOW + "" + UNDERLINE + "" + BOLD + "実績・二つ名メニューへ", RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで移動", "MHF_ArrowLeft")
  ...
  def getHeadingIndex(uuid: UUID, k: MenuInventoryData.MenuType): Option[Integer] = {
    k match {
      case HEAD =>
  ...
  def computeHeadPartCustomMenu(p: Player): Inventory = {
      ...
      if (inventoryIndex < 27) {
        if (playerdata.TitleFlags.contains(i)) {
          ...
          if (maybeHeadPart.nonEmpty) {
            val itemstack = build(Material.WATER_BUCKET, Integer.toString(i), RESET + "" + RED + "前パーツ「" + maybeHeadPart.get + "」")
            AsyncInventorySetter.setItemAsync(inventory, inventoryIndex, itemstack)
            inventoryIndex += 1
          }
        }
        else if (inventoryIndex == 27) { //次ページへのボタンを配置
          val itemstack = buildPlayerSkull(YELLOW + "" + UNDERLINE + "" + BOLD + "次ページへ", RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで移動", "MHF_ArrowRight")
          // 統一性のために右下へ
    ...
    //パーツ未選択状態にするボタン
    // Pure Button
    val itemstack = build(Material.GRASS, YELLOW + "" + UNDERLINE + "" + BOLD + "前パーツを未選択状態にする", RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで実行")
  ...
  private def isError(destination: Player, pd: PlayerData, operation: => String): Boolean = {
    if (pd == null) {
      destination.sendMessage(RED + "playerdataがありません。管理者に報告してください")
      Bukkit.getServer.getConsoleSender.sendMessage(RED + "SeichiAssist[" + operation + "]でエラー発生")
  ...
  def computeMiddlePartCustomMenu(p: Player): Inventory = {
    ...
    for (i <- middlePartIndex(uuid) until 9999) {
      if (inventoryIndex < 27) {
        ...
        if (9911 <= i && playerdata.TitleFlags.contains(i) && maybeMiddlePart.nonEmpty || maybeMiddlePart.nonEmpty) {
          val itemstack = build(Material.MILK_BUCKET, Integer.toString(i), RESET + "" + RED + "中パーツ「" + maybeMiddlePart.get + "」")
          AsyncInventorySetter.setItemAsync(inventory, inventoryIndex, itemstack)
          inventoryIndex += 1
        }
      } else if (inventoryIndex == 27) {
        val lore = Collections.singletonList(RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで移動")
        val itemstack = buildPlayerSkull(YELLOW + "" + UNDERLINE + "" + BOLD + "次ページへ", lore, "MHF_ArrowRight")
    ...
    val itemstack = build(Material.GRASS, YELLOW + "" + UNDERLINE + "" + BOLD + "中パーツを未選択状態にする", RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで実行")
    AsyncInventorySetter.setItemAsync(inventory, 31, itemstack)
  ...
  def computeTailPartCustomMenu(p: Player): Inventory = {
      ...
      if (inventoryIndex < 27) {
        if (playerdata.TitleFlags.contains(i)) {
          ...
          if (maybeTailPart.nonEmpty) {
            val itemstack = build(Material.LAVA_BUCKET, Integer.toString(i), RESET + "" + RED + "後パーツ「" + maybeTailPart.get + "」")
            AsyncInventorySetter.setItemAsync(inventory, inventoryIndex, itemstack)
            inventoryIndex += 1
          }
        } else if (inventoryIndex == 27) {
          val itemstack = buildPlayerSkull(YELLOW + "" + UNDERLINE + "" + BOLD + "次ページへ", Collections.singletonList(RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで移動"), "MHF_ArrowRight")
          AsyncInventorySetter.setItemAsync(inventory, 35, itemstack.clone)
    ...
    val itemstack = build(Material.GRASS, YELLOW + "" + UNDERLINE + "" + BOLD + "後パーツを未選択状態にする", RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで実行")
    AsyncInventorySetter.setItemAsync(inventory, 31, itemstack)
  ...
  def computePartsShopMenu(p: Player): Inventory = { //プレイヤーを取得
    ...
    val lore = util.Arrays.asList(RESET + "" + GREEN + "クリックで情報を最新化", RESET + "" + RED + "累計獲得量：" + ap.cumulativeTotal, RESET + "" + RED + "累計消費量：" + ap.used, RESET + "" + AQUA + "使用可能量：" + ap.left)
    val itemstack = build(Material.EMERALD_ORE, YELLOW + "" + UNDERLINE + "" + BOLD + "実績ポイント 情報", lore)
    AsyncInventorySetter.setItemAsync(inventory, 0, itemstack)
    ...
    for (i <- shopIndex(uuid) to 9833) {
      ...
      if (inventoryIndex < 27) {
        if (!playerdata.TitleFlags.contains(i)) {
          lore = util.Arrays.asList(RESET + "" + RED + "前・後パーツ「" + Nicknames.getHeadPartFor(i).getOrElse(() => "") + "」", RESET + "" + GREEN + "必要ポイント：20", RESET + "" + AQUA + "クリックで購入できます")
          itemstack = build(Material.BEDROCK, Integer.toString(i), lore)
          AsyncInventorySetter.setItemAsync(inventory, inventoryIndex, itemstack)
          inventoryIndex += 1
        } else {
          itemstack = buildPlayerSkull(YELLOW + "" + UNDERLINE + "" + BOLD + "次ページへ", RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで移動", "MHF_ArrowRight")
          AsyncInventorySetter.setItemAsync(inventory, 35, itemstack.clone)
    ...
    for (i <- shopIndex(uuid) to 9938) {
      if (inventoryIndex >= 27) {
        ...
        if (!playerdata.TitleFlags.contains(i)) {
          val lore = util.Arrays.asList(RESET + "" + RED + "中パーツ「" + Nicknames.getMiddlePartFor(i).getOrElse(() => "") + "」", RESET + "" + GREEN + "必要ポイント：35", RESET + "" + AQUA + "クリックで購入できます")
  ...
  def getVotingMenuData(p: Player): Inventory = { //UUID取得
    ...
    val inventory = getEmptyInventory(4, DARK_PURPLE + "" + BOLD + "投票ptメニュー");
    {
      //投票pt受け取り
      val itemstack = build(Material.DIAMOND, LIGHT_PURPLE + "" + UNDERLINE + "" + BOLD + "クリックで投票特典を受け取れます", getVoteButtonLore(playerdata), DIG100)
      AsyncInventorySetter.setItemAsync(inventory, 0, itemstack)
    }

    {
      // ver0.3.2 投票ページ表示
      val lore = util.Arrays.asList(RESET + "" + GREEN + "投票すると様々な特典が！", RESET + "" + GREEN + "1日1回投票出来ます", RESET + "" + DARK_GRAY + "クリックするとチャット欄に", RESET + "" + DARK_GRAY + "URLが表示されますので", RESET + "" + DARK_GRAY + "Tキーを押してから", RESET + "" + DARK_GRAY + "そのURLをクリックしてください")
      val itemstack = build(Material.BOOK_AND_QUILL, YELLOW + "" + UNDERLINE + "" + BOLD + "投票ページにアクセス", lore)
      AsyncInventorySetter.setItemAsync(inventory, 9, itemstack)
    }

    {
      //棒メニューに戻る
      val lore = Collections.singletonList(RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで移動")
      val itemstack = buildPlayerSkull(YELLOW + "" + UNDERLINE + "" + BOLD + "ホームへ", lore, "MHF_ArrowLeft")
      AsyncInventorySetter.setItemAsync(inventory, 27, itemstack.clone)
    }

    {
      //妖精召喚時間設定トグルボタン
      val list = util.Arrays.asList(RESET + "" + GREEN + "" + BOLD + "" + VotingFairyTask.dispToggleVFTime(playerdata.toggleVotingFairy), "", RESET + "" + GRAY + "コスト", RESET + "" + RED + "" + BOLD + "" + playerdata.toggleVotingFairy * 2 + "投票pt", "", RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで切替")
      val itemStack = build(Material.WATCH, AQUA + "" + UNDERLINE + "" + BOLD + "マナ妖精 時間設定", list)
    ...
    {
      //妖精召喚
      val lore = util.Arrays.asList(RESET + "" + GRAY + "" + playerdata.toggleVotingFairy * 2 + "投票ptを消費して", RESET + "" + GRAY + "マナ妖精を呼びます", RESET + "" + GRAY + "時間 : " + VotingFairyTask.dispToggleVFTime(playerdata.toggleVotingFairy), RESET + "" + DARK_RED + "Lv.10以上で解放")
      val itemStack = build(Material.GHAST_TEAR, LIGHT_PURPLE + "" + UNDERLINE + "" + BOLD + "マナ妖精 召喚", lore, DIG100)
      AsyncInventorySetter.setItemAsync(inventory, 4, itemStack)
    }

    if (playerdata.usingVotingFairy) { //妖精 時間確認
      val lore = util.Arrays.asList(RESET + "" + GRAY + "妖精さんはいそがしい。", GRAY + "帰っちゃう時間を教えてくれる")
      val itemStack = build(Material.COMPASS, LIGHT_PURPLE + "" + UNDERLINE + "" + BOLD + "マナ妖精に時間を聞く", lore, DIG100)
      AsyncInventorySetter.setItemAsync(inventory, 13, itemStack)

      val yourRank = playerdata.calcPlayerApple()
      val fairyNotice = List(
        RESET + "" + RED + "" + BOLD + "※ﾆﾝｹﾞﾝに見られないように気を付けること！",
        RESET + "" + RED + "" + BOLD + "  毎日大妖精からデータを更新すること！",
        "",
        RESET + "" + GOLD + "" + BOLD + "昨日までにがちゃりんごを",
        RESET + "" + GOLD + "" + BOLD + "たくさんくれたﾆﾝｹﾞﾝたち",
        RESET + "" + DARK_GRAY + "召喚されたらラッキーだよ！"
      )
      val ranking = (0 to 3.min(SeichiAssist.ranklist_p_apple.size))
        ...
        .collect { case (rankdata, rank) =>
          List(
            GRAY + "たくさんくれたﾆﾝｹﾞﾝ第" + (rank + 1) + "位！",
            GRAY + "なまえ：" + rankdata.name + " りんご：" + rankdata.p_apple + "個"
          )
        }
        .flatten

      val globalInfo = List(
        AQUA + "ぜーんぶで" + SeichiAssist.allplayergiveapplelong + "個もらえた！",
        "",
        GREEN + "↓呼び出したﾆﾝｹﾞﾝの情報↓",
        GREEN + "今までに" + playerdata.p_apple + "個もらった",
        GREEN + "ﾆﾝｹﾞﾝの中では" + yourRank + "番目にたくさんくれる！"
      )
      val lore = fairyNotice ::: ranking ::: globalInfo
      val itemStack = build(Material.GOLDEN_APPLE, YELLOW + "" + UNDERLINE + "" + BOLD + "㊙ がちゃりんご情報 ㊙", lore, DIG100)
  ...
  private def getVotingFairySoundsToggleMeta(playSound: Boolean) = {
    ...
    var lore = null
    itemmeta.setDisplayName(GOLD + "" + UNDERLINE + "" + BOLD + "マナ妖精の音トグル")
    if (playSound)
      lore = util.Arrays.asList(RESET + "" + GREEN + "現在音が鳴る設定になっています。", RESET + "" + DARK_GRAY + "※この機能はデフォルトでONです。", RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで切替")
    else {
      lore = util.Arrays.asList(RESET + "" + RED + "現在音が鳴らない設定になっています。", RESET + "" + DARK_GRAY + "※この機能はデフォルトでONです。", RESET + "" + DARK_RED + "" + UNDERLINE + "クリックで切替")
  ...
  private def getVotingFairyContractMeta(playerdata: PlayerData) = {
    ...
    itemmeta.setDisplayName(GOLD + "" + UNDERLINE + "" + BOLD + "妖精とのお約束")
    // n % 4 + 1 -> 1..4
  ...
  def getGiganticBerserkBeforeEvolutionMenu(p: Player): Inventory = {
    ...
    val inventory = getEmptyInventory(6, DARK_PURPLE + "" + BOLD + "スキルを進化させますか?")
    // 色
    ...
    {
      val lore = util.Arrays.asList(RESET + "" + GREEN + "進化することにより、スキルの秘めたる力を解放できますが", RESET + "" + GREEN + "スキルは更に大量の魂を求めるようになり", RESET + "" + GREEN + "レベル(回復確率)がリセットされます", RESET + "" + RED + "本当に進化させますか?", RESET + "" + DARK_RED + UNDERLINE + "クリックで進化させる")
      val itemstack = build(Material.NETHER_STAR, WHITE + "スキルを進化させる", lore)
      AsyncInventorySetter.setItemAsync(inventory, 31, itemstack)
    }
    inventory
  }
  ...
  def getGiganticBerserkAfterEvolutionMenu(p: Player): Inventory = {
    ...
    val inventory = getEmptyInventory(6, LIGHT_PURPLE + "" + BOLD + "スキルを進化させました")
    val table = Array(12, 15, 4, 0, 3, 12)
    ...
    placeGiganticBerserkShape(inventory);
    {
      val itemstack = build(Material.NETHER_STAR, WHITE + "スキルを進化させました！", util.Arrays.asList(RESET + "" + GREEN + "スキルの秘めたる力を解放することで、マナ回復量が増加し", RESET + "" + DARK_RED + "スキルはより魂を求めるようになりました"))
      AsyncInventorySetter.setItemAsync(inventory, 31, itemstack)
    }
  ...
  private def build[T <: ItemMeta](mat: Material, name: String, lore: util.List[String], modify: Consumer[_ >: T]) = {
    build(mat, name, lore, {x: T => modify.accept(x)})
  }

  private def buildPlayerSkull(name: String, lore: String, owner: String) = buildPlayerSkull(name, Collections.singletonList(lore), owner)

  private def buildPlayerSkull(name: String, lore: util.List[String], owner: String) = buildPlayerSkull(name, lore, owner, nullConsumer[SkullMeta])

  private def buildPlayerSkull(name: String, lore: util.List[String], owner: String, modify: Consumer[_ >: SkullMeta]) = {
  ...
  private def nullConsumer[T]: Consumer[T] = (t: T) => Unit
  ...
  private def placeGiganticBerserkGlass(inv: Inventory, itemstack: ItemStack): Unit = {
    for (i <- Array[Int](6, 7, 14, 15, 16, 21, 22, 23, 24, 32, 41)) {
      AsyncInventorySetter.setItemAsync(inv, i, itemstack)
    }
  }
}
