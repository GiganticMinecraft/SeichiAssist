...
object TrashBox extends ItemConversionStorage {
   ...
   */
  override def doOperation(player: Player, inventory: Map[Int, ItemStack])(implicit environment: this.Environment): IO[ConversionResultSet] = IO.pure(ConversionResultSet.monoid.empty)

  override def doMap(player: Player, itemStack: ItemStack): IO[ConversionResultSet] = IO.pure(ConversionResultSet.monoid.empty)
}
