...
object PlayerSendable {
  ...
  implicit def forStringArray[F[_]: OnMinecraftServerThread]: PlayerSendable[Array[String], F] = {
    (player, content) =>
      OnMinecraftServerThread[F].runAction(SyncIO {
        player.sendMessage(content)
      })
  ...
  implicit def forTextComponent[F[_]: OnMinecraftServerThread]: PlayerSendable[TextComponent, F] = {
    (player, content) =>
      OnMinecraftServerThread[F].runAction(SyncIO {
