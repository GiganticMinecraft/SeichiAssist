  ...
  def dropItemOnTool(
    ...
    if (silkTouch > 0) {
      ...
      blockMaterial match {
        ...
        case Material.WOOD_SLAB | Material.STONE_SLAB | Material.STONE_SLAB2 if (blockDataLeast4Bits & 8) != 0 =>
          // 上付きハーフブロックをそのままドロップするとmissing textureとして描画されるため、下付きの扱いとする
        Some(BlockBreakResult.ItemDrop(new ItemStack(blockMaterial, 1, (blockDataLeast4Bits & 7).toShort)))
        case _ =>
          Some(
            BlockBreakResult
