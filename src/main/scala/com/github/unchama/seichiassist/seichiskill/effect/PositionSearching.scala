...
object PositionSearching {
  ...
  /**
   * [location]を中心としたチェビシェフ距離が[distance]以下の領域に [matchAgainst]の[[XYZTuple]]が一つでも含まれているかを返す。
   */
  def containsOneOfPositionsAround(
    center: XYZTuple,
    distance: Int,
    matchAgainst: Set[XYZTuple]
  ): Boolean = {
    ...
    cuboidToLookFor.gridPoints().map(center + _).exists(matchAgainst.contains)
  }
}
