...
import com.github.unchama.seichiassist.util.{InventoryUtil, StaticGachaPrizeFactory}
...
class MineHeadCommand(implicit ioOnMainThread: OnMinecraftServerThread[IO]) {
  ...
  val effect: TargetedEffect[Player] =
    SequentialEffect(
      InventoryUtil.grantItemStacksEffect(StaticGachaPrizeFactory.getMineHeadItem),
