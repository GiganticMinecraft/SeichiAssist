...
object EffectStatsNotification {
  ...
  def using[
    F[_] : ConcurrentEffect : SendMinecraftMessage[*[_], Player],
    Player
  ](effectDiffWithSettings: fs2.Stream[F, (Player, (EffectListDiff, FastDiggingEffectStatsSettings))]): fs2.Stream[F, Unit] =
    effectDiffWithSettings.evalMap { case (player, (effectListDiff, settings)) =>
