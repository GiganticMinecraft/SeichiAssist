 ...
 */
case class SeichiLevelProgress private (
  progress: Double,
  expAmountToNextLevel: SeichiExpAmount
) {
...
object SeichiLevelProgress {
  ...
  /**
   * 必須経験値量と達成済み経験値量から [[SeichiLevelProgress]] を得る。 必要経験値量は達成済み経験値量未満である必要がある。
   */
  def fromRequiredAndAchievedPair(
    required: SeichiExpAmount,
    achieved: SeichiExpAmount
  ): SeichiLevelProgress = {
    SeichiLevelProgress(
