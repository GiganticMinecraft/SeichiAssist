...
class WorldLevelItemsMigrationVersionRepository[F[_]](serverId: String)(implicit F: Sync[F])
  ...
  override def lockVersionPersistence(
    target: WorldLevelData[F]
  ): Resource[F, PersistenceLock[target.type]] = {
  ...
  override def getVersionsAppliedTo(
    target: WorldLevelData[F]
  ): PersistenceLock[target.type] => F[Set[ItemMigrationVersionNumber]] =
    _ =>
      F.delay {
  ...
  override def persistVersionsAppliedTo(
    target: WorldLevelData[F],
    versions: Iterable[ItemMigrationVersionNumber]
  ): PersistenceLock[target.type] => F[Unit] =
    _ => {
      val batchParams = versions.map { version => Seq(serverId, version.versionString) }.toSeq
      ...
      F.delay {
        ...
        """.batch(batchParams: _*).apply[List]()
        }
      }
    }
}
