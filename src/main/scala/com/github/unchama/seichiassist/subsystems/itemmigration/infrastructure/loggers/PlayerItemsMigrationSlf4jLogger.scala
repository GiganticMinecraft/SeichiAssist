...
class PlayerItemsMigrationSlf4jLogger[F[_]: Sync](logger: Logger)
    extends ItemMigrationLogger[F, PlayerInventoriesData[F]] {
  ...
  override def logMigrationVersionsToBeApplied(
    ...
    Sync[F].delay {
      logger.info(
        s"${target.player.getName} (UUID: ${target.player.getUniqueId})のインベントリ内データ変換を適用します…"
      )
