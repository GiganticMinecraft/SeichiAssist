...
object Util {
   ...
   *   ドロップ率
   * @return
   *   適当な値であれば`rate`、適当な値でなければ`IllegalArgumentException`
   ...
   *   URL
   * @return
   *   適切であれば指定された`url`をそのまま返し、適切でなければ`IllegalArgumentException`を出す
   * @throws IllegalArgumentException
   *   指定されたURLが適切ではない
   */
   ...
   *   期間の終了日
   * @return
   *   期間に含まれるすべてのLocalDateをもつSeq
   * @see
   *   [[https://qiita.com/pictiny/items/357630e48043185da223 Qiita: Scalaで日付の範囲を指定してリストを作る]]
   * @throws IllegalArgumentException
   *   `from`に`to`より後の日付が指定された時
   */
