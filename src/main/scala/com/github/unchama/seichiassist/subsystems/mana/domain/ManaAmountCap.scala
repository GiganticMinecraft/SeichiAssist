...
object ManaAmountCap {
  ...
  val at: SeichiLevel => ManaAmount = {
    /**
     * 整地レベル `level` を引数に取って、 `level` からその次のレベルに上がった際の
     * マナ上限の増加値を返す関数。
     *
     *  - レベル9未満は増加なし
     ...
     *
     * 例えば、
     * レベル10からレベル19までは10ずつ増加し、
     * レベル20からレベル29までは12ずつ、
     * レベル30からレベル39までは16ずつ、
     * レベル40からレベル49までは24ずつ…といった具合に、
     * レベル110までのレベル10ごとに増加幅が2冪で増える。
     */
    ...
    val table = levelsToConsider
      .map(increaseFrom)
      .scanLeft(ManaAmount(0))(_.add(_))
