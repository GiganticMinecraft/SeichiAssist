...
object DragonNightTimeRoutine {
  def apply[
    F[_] : Concurrent : Notifiable : Timer,
    ...
    val effectToAdd = FastDiggingEffect(FastDiggingAmplifier(10.0), FastDiggingEffectCause.FromDragonNightTime)
    ...
    val routineAction: F[Unit] = {
      val manipulateManaMultiplier = Concurrent[F].start {
