...
import com.github.unchama.buildassist.domain.playerdata.{BuildAssistPlayerData, BuildAssistPlayerDataPersistence, CanNotifyLevelUps}
...
class BuildAssistPlayerDataRepository[
  F[_] : SyncEffect : CanNotifyLevelUps[*[_], Player]
  ...
  override protected def initializeValue(player: Player, temporaryData: BuildAssistPlayerData): F[Ref[F, BuildAssistPlayerData]] = {
    ...
    val notifyLevelDiff = levelDiffOption.fold(Monad[F].unit)(CanNotifyLevelUps[F, Player].notifyTo(player))
