...
object MenuInventoryData {
  ...
  def getBlockCraftData(p: Player): Inventory = {
    ...
    val inventory = Bukkit.getServer.createInventory(null, 6 * 9, s"$DARK_PURPLE${BOLD}MineStackブロック一括クラフト1")
    ...
    skullmeta.setDisplayName(s"$YELLOW$UNDERLINE${BOLD}2ページ目へ")
    lore = List(s"$RESET$DARK_RED${UNDERLINE}クリックで移動")
    skullmeta.setLore(lore.asJava)
    skullmeta.setOwner("MHF_ArrowDown")
    itemstack.setItemMeta(skullmeta)
  ...
  def getBlockCraftData2(p: Player): Inventory = {
    ...
    val inventory = Bukkit.getServer.createInventory(null, 6 * 9, s"$DARK_PURPLE${BOLD}MineStackブロック一括クラフト2")
    var itemstack: ItemStack = null
    val skullmeta: SkullMeta = ItemMetaFactory.SKULL.getValue
    var lore: List[String] = null
    ...
    skullmeta.setDisplayName(s"$YELLOW$UNDERLINE${BOLD}1ページ目へ")
    lore = List(s"$RESET$DARK_RED${UNDERLINE}クリックで移動")
    skullmeta.setLore(lore.asJava)
    skullmeta.setOwner("MHF_ArrowUp")
    itemstack.setItemMeta(skullmeta)
    ...
    skullmeta.setDisplayName(s"$YELLOW$UNDERLINE${BOLD}3ページ目へ")
    lore = List(s"$RESET$DARK_RED${UNDERLINE}クリックで移動")
    skullmeta.setLore(lore.asJava)
    skullmeta.setOwner("MHF_ArrowDown")
    itemstack.setItemMeta(skullmeta)
  ...
  def getBlockCraftData3(p: Player): Inventory = {
    ...
    val inventory = Bukkit.getServer.createInventory(null, 6 * 9, s"$DARK_PURPLE${BOLD}MineStackブロック一括クラフト3")
    var itemstack: ItemStack = null
    val skullmeta = ItemMetaFactory.SKULL.getValue
    var lore: List[String] = null
  ...
  private def collectCraftMenu(p: Player, inventory: Inventory, minestackBlockCraftlevel: Int, beforeItemID: NonEmptyList[(String, Int)], afterItemID: NonEmptyList[(String, Int)], minPower10Index: Int, maxPower10Index: Int, inventoryIndex: Int): Unit ={
...

    {
      for (x <- minPower10Index to maxPower10Index) {
		...
		// _1: MS object, _2: power of amount
        val ingredientRequest = ingredients.map(entry => s"${entry._1.uiName.get}${p10 * entry._2}個").mkString("+")
        val lore = List(
          ...
          {products.map(mineStackObjName => s"$RESET$GRAY${mineStackObjName._1.uiName.get}の数:${comma(playerdata_s.minestack.getStackedAmountOf(Util.findMineStackObjectByName(mineStackObjName._1.mineStackObjName)))}")} :::
          List(
