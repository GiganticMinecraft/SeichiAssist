...
object MenuInventoryData {
  ...
  def getBlockCraftData(p: Player): Inventory = {
    ...
    //石を石ハーフブロックに変換10～10万
    batchCraftMenu(p,inventory,1,NonEmptyList.of(("stone",1)), NonEmptyList.of(("step0",2)),1,5,-1)


    //石を石レンガに変換10～1万
    batchCraftMenu(p,inventory,1,NonEmptyList.of(("stone",1)),NonEmptyList.of(("smooth_brick0",1)),1,5,8)

    //花崗岩を磨かれた花崗岩に変換10～1万
    batchCraftMenu(p,inventory,2,NonEmptyList.of(("granite",1)),NonEmptyList.of(("polished_granite",1)),1,4,17)

    //閃緑岩を磨かれた閃緑岩に変換10～1万
    batchCraftMenu(p,inventory,2,NonEmptyList.of(("diorite",1)),NonEmptyList.of(("polished_diorite",1)),1,4,22)

    //安山岩を磨かれた安山岩に変換10～1万
    batchCraftMenu(p,inventory,2,NonEmptyList.of(("andesite",1)),NonEmptyList.of(("polished_andesite",1)),1,4,26)

    //ネザー水晶をネザー水晶ブロックに変換10～1万
    batchCraftMenu(p,inventory,2,NonEmptyList.of(("quartz",4)),NonEmptyList.of(("quartz_block",1)),1,4,31)

    //レンガをレンガブロックに変換10～1万
    batchCraftMenu(p,inventory,2,NonEmptyList.of(("brick_item",4)),NonEmptyList.of(("brick",1)),1,4,35)

    //ネザーレンガをネザーレンガブロックに変換10～1万
    batchCraftMenu(p,inventory,2,NonEmptyList.of(("nether_brick_item",4)),NonEmptyList.of(("nether_brick",1)),1,4,40)
  ...
  def getBlockCraftData2(p: Player): Inventory = {
    ...
    //雪玉を雪（ブロック）に変換10～1万
    batchCraftMenu(p,inventory,2,NonEmptyList.of(("snow_ball",4)),NonEmptyList.of(("snow_block",1)),1,4,-1)

    //ネザーウォートとネザーレンガを赤いネザーレンガに変換10～1万
    batchCraftMenu(p,inventory,2,NonEmptyList.of(("nether_stalk",2),("nether_brick_item",2)),NonEmptyList.of(("red_nether_brick",1)),1,5,4)

    //石炭を消費して鉄鉱石を鉄インゴットに変換4～4000
    batchCraftMenu(p,inventory,3,NonEmptyList.of(("iron_ore",4),("coal",1)),NonEmptyList.of(("iron_ingot",4)),0,3,9)

    //溶岩バケツを消費して鉄鉱石を鉄インゴットに変換50～5万
    batchCraftMenu(p,inventory,3,NonEmptyList.of(("iron_ore",50),("lava_bucket",1)),NonEmptyList.of(("iron_ingot",50)),0,3,14)

    //石炭を消費して金鉱石を金インゴットに変換4～4000
    batchCraftMenu(p,inventory,3,NonEmptyList.of(("gold_ore",4),("coal",1)),NonEmptyList.of(("gold_ingot",4)),0,3,18)

    //溶岩バケツを消費して金鉱石を金インゴットに変換50～5万
    batchCraftMenu(p,inventory,3,NonEmptyList.of(("gold_ore",50),("lava_bucket",1)),NonEmptyList.of(("gold_ingot",50)),0,3,23)

    //石炭を消費して砂をガラスに変換4～4000
    batchCraftMenu(p,inventory,3,NonEmptyList.of(("sand",4),("coal",1)),NonEmptyList.of(("glass",4)),0,3,27)

    //溶岩バケツを消費して砂をガラスに変換50～5万
    batchCraftMenu(p,inventory,3,NonEmptyList.of(("sand",50),("lava_bucket",1)),NonEmptyList.of(("glass",50)),0,3,32)

    //石炭を消費してネザーラックをネザーレンガに変換4～4000
    batchCraftMenu(p,inventory,3,NonEmptyList.of(("netherrack",4),("coal",1)),NonEmptyList.of(("nether_brick_item",4)),0,3,36)

    //溶岩バケツを消費してネザーラックをネザーレンガに変換50～5万
    batchCraftMenu(p,inventory,3,NonEmptyList.of(("netherrack",50),("lava_bucket",1)),NonEmptyList.of(("nether_brick_item",50)),0,3,41)
  ...
  def getBlockCraftData3(p: Player): Inventory = {
    ...
    batchCraftMenu(p,inventory,3,NonEmptyList.of(("clay_ball",4),("coal",1)),NonEmptyList.of(("brick_item",4)),0,3,0)

    //溶岩バケツを消費して粘土をレンガに変換50～5万
    batchCraftMenu(p,inventory,3,NonEmptyList.of(("clay_ball",50),("lava_bucket",1)),NonEmptyList.of(("brick_item",50)),0,3,5)
