...
object MenuInventoryData {
  ...
  def getSetBlockSkillData(p: Player): Inventory = {
    ...
    val inventory =
      Bukkit.getServer.createInventory(null, 4 * 9, s"$DARK_PURPLE$BOLD「範囲設置スキル」設定画面")
    ...
    lore =
      List(s"$RESET$AQUA${UNDERLINE}機能の使用設定：$ZSDirt", s"$RESET$AQUA${UNDERLINE}機能の範囲：地下5マスまで")
    itemmeta.setLore(lore.asJava)
    itemstack.setItemMeta(itemmeta)
