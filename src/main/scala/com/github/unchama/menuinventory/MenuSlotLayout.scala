...
case class MenuSlotLayout(private[menuinventory] val layoutMap: Map[Int, Slot]) {
  ...
  /**
   * @return
   *   クリックされた枠に対応した[Slot]が[InventoryClickEvent]に基づいて引き起こす作用
   */
  def effectOn(
   ...
   */
  def merge(another: MenuSlotLayout): MenuSlotLayout = MenuSlotLayout(
    layoutMap ++ another.layoutMap
  )

  /**
   * [slotReplacement]でレイアウトの一箇所を置き換えた新しいレイアウトを計算する.
   */
  def altered(slotReplacement: (Int, Slot)): MenuSlotLayout =
    copy(layoutMap = layoutMap + slotReplacement)
}
...
object MenuSlotLayout {
  ...
  @inline def singleSlotLayout(indexedSlot: => (Int, Slot)): MenuSlotLayout = MenuSlotLayout(
    indexedSlot
  )
