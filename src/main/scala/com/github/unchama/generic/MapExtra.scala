...
object MapExtra {
   ...
   *
   * 任意の `g: K => T`、 `k: K`、 `map: Map[K, V]` について、
   * `collapseKeys(g)(map).get(k) == map.filterKeys(k1 => g(k) == g(k1)).combineAll`
   * を満たす。
   */
  def collapseKeysThrough[T, K, V: CommutativeSemigroup](grouping: K => T)
                                                        (map: Map[K, V]): Map[K, V] =
    map
      .groupBy { case (k, _) => grouping(k) }
      .toList
      .mapFilter { case (_, groupedMap) =>
        groupedMap.toList match {
          case ::((k, v), remaining) => Some {
