...
object ItemMigrationStateRepositoryDefinitions {
  ...
  def initialization[F[_] : Sync]: SinglePhasedRepositoryInitialization[F, PlayerMigrationState[F]] =
    SinglePhasedRepositoryInitialization.withSupplier(PlayerMigrationState.newIn[F])

  def finalization[F[_] : Applicative, Player]: RepositoryFinalization[F, Player, PlayerMigrationState[F]] =
