...
public class AchieveCommand implements TabExecutor{
	...
	String label, String[] args) {
		...
		//不正な数の引数を指定した場合(2より小さい場合 or 3より大きい場合 →lengthが2～3以外の場合)
			...
			sender.sendMessage(ChatColor.RED + "/unlockachv <実績No> <プレイヤー名> <server/world/null>");
			sender.sendMessage("【HINT】<プレイヤー名>を「ALL」にし、最後に<server/world>を入力すると");
		...
		//<実績No>が数字でない場合スキップ
			...
			sender.sendMessage(ChatColor.RED + "/unlockachv <実績No> <プレイヤー名> <server/world>");
			return true;
		}
	}
}
