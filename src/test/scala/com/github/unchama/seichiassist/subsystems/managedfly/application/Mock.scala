...
private[managedfly] class Mock[AsyncContext[_]: Concurrent, SyncContext[
  ...
  case class FiniteNonNegativeExperience(internalAmount: BigInt) extends ExperienceMock {
    ...
    override def consume(amount: BigInt): Option[ExperienceMock] =
      if (amount <= internalAmount) Some(FiniteNonNegativeExperience(internalAmount - amount))
      else None
  }
  ...
  case class StatusMessageMock(
    idleStatus: IdleStatus,
    remainingFlyDuration: RemainingFlyDuration
  ) extends MessageMock

  import ContextCoercion._
  import cats.implicits._

  case class PlayerMockReference(
    isFlyingMutex: Mutex[AsyncContext, SyncContext, Boolean],
    ...
    def sendMessage(message: MessageMock): AsyncContext[Unit] = {
      messageLog
        .lockAndUpdate { current => Monad[AsyncContext].pure(current.appended(message)) }
        .as(())
    }
  }
  ...
  def playerMockFlyStatusManipulation(
